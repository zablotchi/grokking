...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, mask, past_kv=None):
        ...
        ), "keys and values time dimension must match"
        assert (
            past_kv is None or past_kv[0].shape[1] == past_kv[1].shape[1]
        ...
        val_heads = self.val_proj(values).reshape(
            batch, time, self.heads, self.attn_dim
        )
        query_heads = self.query_proj(values).reshape(
            batch, time, self.heads, self.attn_dim
        )
        ...
        attn_matrix = F.softmax(
            (
                torch.einsum("bqhd,bkhd->hbqk", query_heads, key_heads)
...
class TransformerBlock(nn.Module):
    def __init__(
        self, heads, hidden_dim, attn_dim, intermediate_dim, dropout=0.1, pre_norm=True
    ):
    ...
    def forward(self, x, attn_mask, past_kv=None):
        if not self.pre_norm:
            attn_output, attn_matrix, past_kv = self.attn(
            ...
            attn_output, attn_matrix, past_kv = self.attn(
                x_norm1, x_norm1, x_norm1, attn_mask, past_kv=past_kv
            )
